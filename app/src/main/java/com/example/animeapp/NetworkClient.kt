...
class NetworkClient {
    ...
    fun getAnimeListByGenre(genreId: Int, callback: (List<Anime>) -> Unit) {
        val url = "$baseUrl/anime?genres=$genreId"
        ...
        client.get(url, object : JsonHttpResponseHandler() {
            override fun onSuccess(statusCode: Int, headers: okhttp3.Headers?, response: JSON?) {
                try {
                    ...
                    // Log the size of the fetched anime list
                    Log.d("NetworkClient", "Fetched ${animeList.size} anime for genre ID: $genreId")

                    callback(animeList.take(10)) // Get only the first 10 anime

                } catch (e: Exception) {
                    Log.e("NetworkClient", "Error fetching anime by genre", e)
                }
            }
